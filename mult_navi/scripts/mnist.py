...
train_dataset = datasets.MNIST(root='./mnist', train=True, transform=transform)  # 本地没有就加上download=True
test_dataset = datasets.MNIST(root='./mnist', train=False, transform=transform)  # train=True训练集，=False测试集
...
#     plt.title("Labels: {}".format(train_dataset.targets[i])) #train_labels
#     plt.xticks([])
#     plt.yticks([])
# plt.show()
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        torch.save(model, './model_Mnist.pth')
